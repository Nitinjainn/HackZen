...
export default function CreateEventEnhanced() {
  ...
  function openDateTimePicker(input: HTMLInputElement | null) {
    if (!input) return;
    try {
      // @ts-expect-error: showPicker is not yet in TS lib
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ref={startsAtRef}
                        type="datetime-local"
                        className="w-full bg-black border border-gray-700 rounded-xl px-4 py-3 pr-12 text-white focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20 transition-all duration-300 [&::-webkit-calendar-picker-indicator]:filter [&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:brightness-0 [&::-webkit-calendar-picker-indicator]:contrast-1000"
                        value={startsAt}
                      ...
                      <button
                        ...
                        onClick={() => openDateTimePicker(startsAtRef.current)}
                        className="absolute inset-y-0 right-3 my-auto h-8 w-8 inline-flex items-center justify-center rounded-md text-white hover:text-cyan-400 hover:bg-gray-800"
                      >
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ref={endsAtRef}
                        type="datetime-local"
                        className="w-full bg-black border border-gray-700 rounded-xl px-4 py-3 pr-12 text-white focus:border-purple-400 focus:ring-2 focus:ring-purple-400/20 transition-all duration-300 [&::-webkit-calendar-picker-indicator]:filter [&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:brightness-0 [&::-webkit-calendar-picker-indicator]:contrast-1000"
                        value={endsAt}
                      ...
                      <button
                        ...
                        onClick={() => openDateTimePicker(endsAtRef.current)}
                        className="absolute inset-y-0 right-3 my-auto h-8 w-8 inline-flex items-center justify-center rounded-md text-white hover:text-purple-400 hover:bg-gray-800"
                      >
                        <Calendar className="w-5 h-5" />
                      </button>
                    </div>
